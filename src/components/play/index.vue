<template>
  <div class="play">
    ...
    <div
      class="song-bg"
      :style="
        `background-image: url(${picUrl}?imageView&thumbnail=360y360&quality=75&tostatic=0);`
      "
    ></div>
    <!-- 播放页头部导航 -->
    <div class="header">
    ...
    <div class="song-wrapper">
      ...
      >
        <div class="song-img">
          <img
            style="width: 100%"
            :src="`${picUrl}?imageView&thumbnail=360y360&quality=75&tostatic=0`"
            alt=""
          />
        </div>
      </div>
...
<script>
// 获取歌曲详情和 歌曲的歌词接口
import { getSongById, getLyricById } from '@/api/server'
export default {
  name: 'MyPlay',
  data () {
    return {
  ...
  computed: {
    ...
    picUrl () {
      return this.songInfo && this.songInfo.al && this.songInfo.al.picUrl
    }
  },
  methods: {
    // 获得歌曲详情的方法，调用它会获得歌曲内容和歌词
    async getSong () {
